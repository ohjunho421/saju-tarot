...
[질문에 대한 결론]
⚠️ 매우 중요: 결론은 반드시 위에 뽑힌 카드들의 의미를 종합해서 도출하세요!
- 부정적/역방향 카드가 많으면 → "지금은 피하세요", "시기상조입니다", "어려움이 예상됩니다" 등 솔직하게
- 긍정적/정방향 카드가 많으면 → "좋은 시기입니다", "진행하세요", "기회입니다" 등 명확하게
- "신중하게 생각하세요", "잘 판단하세요" 같은 모호하고 일반적인 답변 절대 금지!
- 카드가 말하는 메시지를 그대로 전달하세요. 타로는 점이니까 명확한 답을 주세요.
${userName ? userName + '님' : '당신'}의 질문에 대해 카드들이 말하는 핵심 답변 (150~200자)
